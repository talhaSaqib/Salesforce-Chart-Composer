...
function resolveModuleFromDir(specifier, moduleRecord, opts) {
    ...
    const absModuleDir = path_1.default.isAbsolute(dir) ? dir : path_1.default.join(opts.rootDir, dir);
    if (!fs_1.default.existsSync(absModuleDir)) {
        throw new errors_1.LwcConfigError(`Invalid dir module record "${JSON.stringify(moduleRecord)}", directory ${absModuleDir} doesn't exists`, { scope: opts.rootDir });
    }
    ...
    const [ns, name] = parts;
    const moduleDir = path_1.default.join(absModuleDir, ns, name);
